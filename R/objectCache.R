  ...
  function(entity, objectName)
{
  ...
  save(list = objectName, envir=as.environment(entity$objects), file = destFile)
}
...
.generateCacheFileName <- 
  function(entity, objectName, cacheSubDir = synapseObjectCache())
{
  kSuffix <- "rbin"
...
.loadCachedObjects <-
  function(entity, cacheSubDir = synapseObjectCache())
{
  lapply(
    list.files(file.path(entity$cacheDir, cacheSubDir), full.names=T, pattern = "rbin$"),
    FUN = function(filepath){
      load(filepath, envir = as.environment(entity$objects))
    }
  )
  invisible(entity)
}
