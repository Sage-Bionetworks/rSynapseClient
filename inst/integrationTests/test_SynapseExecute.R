...
#   createdFolder<-synapseClient:::getOrCreateEntity(name="test folder", parentId=propertyValue(project, "id"), entityType="Folder")
#   synapseClient:::.setCache("testFolder", createdFolder)
#   firstId <- propertyValue(createdFolder, "id")
#   checkTrue(!is.null(firstId))
#   checkEquals("test folder", propertyValue(createdFolder, "name"))
#   checkTrue("Folder"==class(createdFolder))
#   # now verify that if we try to get/create the same folder, we get the same entity, not a new one
#   createdFolder2<-synapseClient:::getOrCreateEntity(name="test folder", parentId=propertyValue(project, "id"), entityType="Folder")
#   secondId <- propertyValue(createFolder2, "id")
...
#   checkEquals(synapseClient:::indent(rawToChar(getURLContent(codeUrl, .opts=synapseClient:::.getCache("curlOpts")))), propertyValue(createdEntity, "description"))
#   
