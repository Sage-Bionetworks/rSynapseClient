...
\end{verbatim}
The following example shows how to add a new row to a Table.  First, define a new column:
\begin{verbatim}
newColumn<-TableColumn(name="new", columnType="STRING")
newColumn<-synStore(newColumn)
\end{verbatim}
Now add the new column to the existing schema:
\begin{verbatim}
propertyValue(schema, "columnIds")<-c(propertyValue(schema, "columnIds"), newColumn$id)
schema<-synStore(schema)
\end{verbatim}
When the table is retrieved, the values in the new column are NA:
\begin{verbatim}
table<-synTableQuery(sprintf("select * from %s", propertyValue(schema, "id")), loadResult=TRUE)
table@values
\end{verbatim}
